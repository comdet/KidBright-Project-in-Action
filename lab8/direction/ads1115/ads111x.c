...
#define READ_CONFIG(OFFS, MASK, VAR, TYPE) do { \
        CHECK_ARG(VAR); \
        uint16_t bits; \
...
{
    READ_CONFIG(OS_OFFSET, OS_MASK, busy, bool);
}
...
{
    READ_CONFIG(PGA_OFFSET, PGA_MASK, gain, ads111x_gain_t);
}
...
{
    READ_CONFIG(MUX_OFFSET, MUX_MASK, mux, ads111x_mux_t);
}
...
{
    READ_CONFIG(MODE_OFFSET, MODE_MASK, mode, ads111x_mode_t);
}
...
{
    READ_CONFIG(DR_OFFSET, DR_MASK, rate, ads111x_data_rate_t);
}
...
{
    READ_CONFIG(COMP_MODE_OFFSET, COMP_MODE_MASK, mode, ads111x_comp_mode_t);
}
...
{
    READ_CONFIG(COMP_POL_OFFSET, COMP_POL_MASK, polarity, ads111x_comp_polarity_t);
}
...
{
    READ_CONFIG(COMP_LAT_OFFSET, COMP_LAT_MASK, latch, ads111x_comp_latch_t);
}
...
{
    READ_CONFIG(COMP_QUE_OFFSET, COMP_QUE_MASK, queue, ads111x_comp_queue_t);
}
